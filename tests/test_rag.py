...
@patch('app.services.rag_pipeline.search_similar_chunks', new_callable=AsyncMock)
@patch('app.services.rag_pipeline.fetch_document_parts', new_callable=AsyncMock)
@pytest.mark.asyncio
async def test_retrieve_context(mock_fetch, mock_search, mock_embedder):
    # Mocking embedder
    ...
    mock_fetch.return_value = [
        {"source_doc_id": "doc_1", "title": "Test Title 1", "root_title": "Root Title 1", "full_markdown": "Full mega-chunk 1 text"},
        {"source_doc_id": "doc_2", "title": "Test Title 2", "root_title": "Root Title 2", "full_markdown": "Full mega-chunk 2 text"}
    ]
    
    result = await retrieve_context("What is the civil code?", top_k=3)
...
@patch('app.services.rag_pipeline.fetch_document_parts', new_callable=AsyncMock)
@pytest.mark.asyncio
async def test_retrieve_context_no_results(mock_fetch, mock_search, mock_embedder):
    mock_embed_instance = AsyncMock()
    ...
    result = await retrieve_context("Unknown question", top_k=5)
